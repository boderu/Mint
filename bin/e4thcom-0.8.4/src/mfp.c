/* mfp.c  for MinForth Plus (derived from mf.c)
  --------------------------------------------------------------------------
  ...
  MM-190402 Some modification required for OSX
...
/* ===========================================================================
   MF:
      ...
      in the command line.
   Command line:
      mf [-i imagefile] [Forth command line]  \ MM-130415 \i changed to -i
...
char	appdir[128]="";		/* MinForth appdir */
...
#define NAMES	  4*CellSize  // 16		/* Namespace address */     MM-32/64
#define HEAP	  5*CellSize  // 20		/* Heapspace address */
#define LIMIT	  6*CellSize  // 24		/* last Forthspace adress */
#define THROW_CFA 10*CellSize // 40		/* Hilevel-Throw-CFA */
#define DEBUG_CFA 11*CellSize // 44		/* Debug-Trace-CFA */
#define HL_CLOCK  12*CellSize // 48		/* Hilevel clock for multitasking */
#define CODE_DP   13*CellSize // 52		/* Hilevel HERE */
#define NAME_DP   14*CellSize // 56		/* Hilevel HERE-N */
...
#define NEXTW	   	W+sizeof(Cell)    //   W+4                      MM-32/64
#define IP_NEXT 	IP+=sizeof(Cell)  //   IP+=4
#define SETIP(x)	IP=(x)
...
char GetKey(void) /* get or wait for next keyboard event */
{
    ...
    if (pending == -1)
       do
         usleep(10000); /* for Linux ok, but for Minix? | changed from 50000 */
       while (WaitKey() == 0);    /* to 10000  MM-091225                     */
...
Cell ReadCell()
{  Cell value;
// fread(&value,4,1,imagefile);                                     MM-32/64
   fread(&value,CellSize,1,imagefile);
   return(value);
}
...
// fseek(imagefile,(12+16),SEEK_SET);                               MM-32/64
   fseek(imagefile,((3+4)*CellSize),SEEK_SET);
...
// Forthspace  = (Char Mfar*)Mcalloc(totalsize+4,sizeof(Char));     MM-32/64
   Forthspace  = (Char Mfar*)Mcalloc(totalsize+CellSize,sizeof(Char));
...
// fseek(imagefile,12,SEEK_SET); /* set fptr to start of codeimage */
   fseek(imagefile,(3*CellSize),SEEK_SET); /* set fptr to start of codeimage */
...
{  int i,st; Addr adr;
 #if _OSTYPE == 3 /*  == 3   old Linux version   MM-190402 OSX */
   /* MM-110320 Read path to the executable from the /proc file system */
   sprintf(fname,"/proc/%d/exe",getpid());
   fname[readlink(fname,fname,126)]='\0';
   /* MM-130314 remove executable name from the path */
   fname[(strlen(fname)-strlen(strrchr(fname,'/')))]='\0';
   strcpy(appdir,fname);  /* MM-190606 OSX */
   /* printf("appdir:%s\n",fname); fflush(stdout); */
 #endif

 #if _OSTYPE == 300 /*  == 3   Linux and OSX version   MM-190402 OSX */
   /* Read path to the executable using realpath(). */
   char *npx;
   npx=realpath(argv[0],NULL); strncpy(fname,npx,126); free(npx);
   /* MM-130314 remove executable name from the path */
   fname[(strlen(fname)-strlen(strrchr(fname,'/')))]='\0';
   strcpy(appdir,fname);
   /* printf("mfp.c: appdir=%s\n",appdir); fflush(stdout); */
 #endif

/*   if ((argc >=3)&&(argv[1][0]=='/')&&(toupper(argv[1][1])=='I'))  MM-130415 */
   if ((argc >=3)&&(argv[1][0]=='-')&&(toupper(argv[1][1])=='I'))
   {  strcpy(imagename,argv[2]);
      if (NULL == strchr(imagename,'.')) strcat(imagename,".i");
   #if _OSTYPE == 3
      /* MM-130414 Append image name to the path */
      strcat(fname,"/");
      strcat(fname,imagename);
      /* printf("image:%s\n",fname); fflush(stdout); */
   #else
      strcpy(fname,imagename);  /* MM-110320 */
   #endif
   st = 3;
   }
   else
   ...
   #if _OSTYPE == 3
      /* MM-130414 */
      /* Read image name from argv[0] */ 
      if (NULL == strchr(argv[0],'/'))
        { strcpy(imagename,"/") ;
         strcat(imagename,argv[0]); }
      else
        { strcpy(imagename,(strrchr(argv[0],'/'))); }
      /* Append image name to the path */
      strcat(fname,imagename);
      /* */
      strcat(fname,".i");  /* MM-110320 */
      /* printf("image:%s\n",fname); fflush(stdout); */
   #else
      char *np1, *np2;
...
#if _OSTYPE <= 2
/* imagefile = fopen(imagename,"rb");  MM-110320 */
   imagefile = fopen(fname,"rb");
#else
/* imagefile = fopen(imagename,"r");   MM-110320 */
   imagefile = fopen(fname,"r");
#endif
   if (imagefile == NULL)
   {  strcat(fname," imagefile open failed");
      Terminate(fname); }
// if ((Cell)0xe8f4b4bd != ReadCell()) /* check magic number        MM-32/64
   if ((Cell)-386616131 != ReadCell()) /* check magic number */
      strcat(fname," is no MinForth imagefile"), Terminate(fname);
   ReadImageFile();
   fname[0]='\0';
   for (i=st; i<argc; i++) {
       /* MM-130312 */
       if (sizeof(fname) < (strlen(fname)+strlen(argv[i])+1))
         Terminate("command line overflow");
       /* */
...
// hdr = AT(NAMES) + 4, hdrmax = AT(NAMES) + AT(NAME_DP);            MM-32/64
   hdr = AT(NAMES) + CellSize, hdrmax = AT(NAMES) + AT(NAME_DP);
   while (hdr < hdrmax)
   {  cfa = (Addr)AT(hdr+CellSize),  clen = 0xffff & (AT(hdr+(2*CellSize)));
   // cfa = (Addr)AT(hdr+4),  clen = 0xffff & (AT(hdr+8));           MM-32/64
   // if (((clen==0)&&(cfa==ca))||((ca>=cfa-4)&&(ca<cfa-4+clen)))    MM-32/64
      if (((clen==0)&&(cfa==ca))||((ca>=cfa-CellSize)&&(ca<cfa-CellSize+clen)))
	 return(hdr);
   // hdr += ((0x1f & (CAT(hdr+12))) + 17) & -4l; }                  MM-32/64
      hdr += ((0x1f & (CAT(hdr+(3*CellSize)))) + (4*CellSize+1)) & -CellSize; }
   return(0);
}
...
{  Int stkd, fltd, rstd, i; Addr r, hdr; Char Mfar* na;
   ...
   {  r = rst[i];
      ...
      if (hdr)
      {  na = PTR(hdr+13);
//	 if (*na) fprintf(stderr," --> %s + %ld",na,r-AT(hdr+4));        MM-32/64
	 if (*na) fprintf(stderr," --> %s + %ld",na,r-AT(hdr+CellSize));
//	 else fprintf(stderr," --> XT%ld + %ld",AT(hdr+4),r-AT(hdr+4));  MM-32/64
	 else fprintf(stderr," --> XT%ld + %ld",AT(hdr+CellSize),r-AT(hdr+CellSize));
//	 hdr = SearchNames(AT(r-4));                                     MM-32/64
	 hdr = SearchNames(AT(r-CellSize));
	 na = PTR(hdr+13);
	 if (*na) fprintf(stderr," --> %s",na);
//   else fprintf(stderr," --> XT%ld",AT(hdr+4)); } }                MM-32/64
	 else fprintf(stderr," --> XT%ld",AT(hdr+CellSize)); } }
   fprintf(stderr,"\n(press Return)");
   getchar();
...
// if (stk-stk_max < 4) stk = stk_max+4;                             MM-32/64
   if (stk-stk_max < CellSize) stk = stk_max+CellSize;
// if (rst-rst_max < 4) rst = rst_max+4;                             MM-32/64
   if (rst-rst_max < CellSize) rst = rst_max+CellSize;
//   if ((A >= 256) && (A < totalsize)) {                            MM-32/64
   if ((A >= (64*CellSize)) && (A < totalsize)) {
      PUSH(code), W = A;
      AT(THROW_CFA)=0;
      ExecuteToken();
      AT(THROW_CFA)=A;
...
// if (a & 3) Throw(-23);                                           MM-32/64
   if (a & (sizeof(Cell)-1)) Throw(-23);
   IP = a;
}
void Indepth(Int d)
...
{  Addr A = At(NEXTW);
// if ((A < 16)||(A >= 256)) Throw(-9);                             MM-32/64
   if ((A < 16)||(A >= (64*CellSize))) Throw(-9);
   Push(A);
}
...
void pFLIT() /* ( f: -- r ) push inline float onto float stack */
// {  FPush(*(Float Mfar*)(PTR(IP))), IP += 8;                 //     MM-32/64
{  FPush(*(Float Mfar*)(PTR(IP))), IP += (sizeof(Float));
}

void pSLIT() /* ( -- adr len ) push inline string address and length */
{  Char Len = CAT(IP);
   Push(IP+1), Push(Len);
// SETIP((IP+Len+5) & -4l);                                         MM-32/64
   SETIP((IP+Len+(CellSize)+1) & -(CellSize));
}
...
#if _OSTYPE == 3  /* MM-110228 */
void pATX() /* ( adr -- x ) read value x from external address adr */
{ Indepth(1);
  TOS = (*(Cell*)(TOS)); 
}
#endif
...
// memmove(stk+1,stk,4*i); TOS = ni;                                 MM-32/64
   memmove(stk+1,stk,CellSize*i); TOS = ni;
}
...
{
/*    printf(" mu* MATH_64=%u ",MATH_64);       MM-32/64  */
#if MATH_64
   Addr u; DAddr D1,DRes;
   Indepth(3); u=POP(); DStore(&D1,TOS,SECOND);
   DRes = D1 * u;
   SECOND = DLo(DRes), TOS = DHi(DRes);
#else                  /* changed from 32 to 64 bit  MM-32/64 */
   Addr u, ul, uh, al, ah, a, b;
   Indepth(3); u = POP(), a = SECOND, b = TOS;
// ah = a >> 16, al = a & 0xffff, uh = u >> 16, ul = u & 0xffff;    MM-32/64
   ah = a >> 32, al = a & 0xffffffff, uh = u >> 32, ul = u & 0xffffffff;
   SECOND = a * u, TOS = b * u + ah * uh;
// a = uh * al, b = a + ul * ah + ((ul * al)>>16),                  MM-32/64
   a = uh * al, b = a + ul * ah + ((ul * al)>>32),
// u = b >> 16; if (a > b) u |= 0x10000l;                           MM-32/64
   u = b >> 32; if (a > b) u |= 0x100000000l;
   TOS += u;
#endif
}
...
{
/*   printf("mu/mod MATH_64=%u ",MATH_64);   */
#if MATH_64
   Addr u; DAddr D1, DRes;
   ...
   if (u == 0) Throw(-10);
   a = c / u, c = c % u;
// for (i=0; i<64; i++)                                             MM-32/64
   for (i=0; i<(sizeof(Cell)*8); i++)
   {   sa =	       c >> (sizeof(Cell)*8-1);
       c = (c << 1) | (b >> (sizeof(Cell)*8-1));
       b = (b << 1) | (a >> (sizeof(Cell)*8-1));
       a =  a << 1;
       if (sa | (c >= u)) c -= u, a += 1; }
...
#if _OSTYPE == 3   /* MM-100912  */
   if (f&4000)   u |= (O_NONBLOCK);
   if (f&2000)   u |= (O_APPEND);  /* MM-110315 */
#endif
...
{  Addr buf; char* errmsg; Int u;
   Indepth(2); buf = POP();
   errmsg = GetErrorMessage((int)TOS);
...
/* Next code line replaced. It failed because of a wrong dest-address
	  CAT(buf) = (Char)u; strcpy((char*)(buf+1),env); */
          CAT(buf) = (Char)u; strcpy((char Mfar*)PTR(buf+1),env);
/* MM-110204 */
	  TOS = MTRUE; }
}
...
void pRESIZEFORTH() /* ( addon -- ior ) resize heapspace */
{  Char Mfar* NewForthspace; Cell a;
// Indepth(1); a = (7 + TOS) & -4;                                  MM-32/64
   Indepth(1); a = (2*CellSize - 1 + TOS) & -CellSize;
...
{  Addr a, u, cslfa, pcslfa, cura;
   ...
   {  cura = At(pcslfa);
      ...
      Inrange(cslfa,44);
//    cura = 0x1f & (CAT(cslfa+12));                                MM-32/64
      cura = 0x1f & (CAT(cslfa+(3*CellSize)));
//    if ((u==cura) && !memcmp(PTR(a),PTR(cslfa+13),u))             MM-32/64
      if ((u==cura) && !memcmp(PTR(a),PTR(cslfa+(3*CellSize + 1)),u))
...
/* ------------------------------------------------------------------------
   Simple API function interface for Windows PCs
*/
...
void p_KERNEL32() /* ( -- hdll ) get kernel library handle */
{  if (kernel32 == 0)
      kernel32 = (Cell)GetModuleHandle((LPCSTR)"kernel32.dll");
   Push(kernel32);
}

void p_LOADDLL() /* ( far_name -- hdll|0 ) get library handle */
{  Cell p;
   Indepth(1);
   p = (Cell)LoadLibrary((LPCTSTR)TOS);
   if (p == 0) Throw(-261);
   TOS = p;
}

void p_GETPROC() /* ( far_name hdll -- hproc|0 ) get procedure handle */
{  HMODULE dll; Cell p;
   Indepth(2); dll = (HMODULE)POP();
   p = (Cell)GetProcAddress(dll,(LPCSTR)TOS);
   if (p == 0) Throw(-261);
   TOS = p;
}
...
Cell (*pz)();
Cell (*pn)(Cell,...);
void p_RUNPROC() /* ( xn..x1 prochandle n -- y ) run Win32 API procedures */
{  Cell x1, x2, x3, x4, x5;
   x1 = Pop(); /* printf("%ld",x1); fflush(stdout); */
   switch(x1)
   {  case 0:  Indepth(1);
               pz=(void*)TOS;
               TOS=pz();
      ...
      case 1:  Indepth(2);
               ...
               break;
      case 2:  Indepth(3);
               pn=(void*)POP(), x2=POP(), x1=TOS;
               TOS=pn(x1,x2);
               break;
      case 3:  Indepth(4);
               pn=(void*)POP(), x3=POP(), x2=POP(), x1=TOS;
               TOS=pn(x1,x2,x3);
               break;
      case 4:  Indepth(5);
               pn=(void*)POP(), x4=POP(), x3=POP(), x2=POP(), x1=TOS;
               TOS = pn(x1,x2,x3,x4);
               break;
      case 5:  Indepth(6);
               pn=(void*)POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=TOS;
               TOS = pn(x1,x2,x3,x4,x5);
               break;
      default: Throw(-21); }
}

/* Alternative version, thanks to Klaus Seegebarth */
/*
typedef  __declspec(dllimport) __stdcall Cell (*FCTPTR1)(Cell); 
typedef  __declspec(dllimport) __stdcall Cell (*FCTPTR2)(Cell,Cell); 
typedef  __declspec(dllimport) __stdcall Cell (*FCTPTR3)(Cell,Cell,Cell); 
typedef  __declspec(dllimport) __stdcall Cell (*FCTPTR4)(Cell,Cell,Cell,Cell); 
typedef  __declspec(dllimport) __stdcall Cell (*FCTPTR5)(Cell,Cell,Cell,Cell,Cell); 
Cell (*pz)();
FCTPTR1 pn1;
FCTPTR2 pn2;
FCTPTR3 pn3;
...
{  Cell x1, x2, x3, x4, x5;
   ...
   {  case 0:  Indepth(1);
      ...
      case 1:  Indepth(2);
	       ...
	       break;
      case 2:  Indepth(3);
               pn2=(FCTPTR2)POP(), x2=POP(), x1=TOS;
               TOS=pn2(x1,x2);
               break;
      case 3:  Indepth(4);
               pn3=(FCTPTR3)POP(), x3=POP(), x2=POP(), x1=TOS;
               TOS=pn3(x1,x2,x3);
               break;
      case 4:  Indepth(5);
               pn4=(FCTPTR4)POP(), x4=POP(), x3=POP(), x2=POP(), x1=TOS;
               TOS = pn4(x1,x2,x3,x4);
               break;
      case 5:  Indepth(6);
               pn5=(FCTPTR5)POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=TOS;
               TOS = pn5(x1,x2,x3,x4,x5);
               break;
      default: Throw(-21); }
}
*/
...
/* ----- extras for Linux MM-091223 ----------------- */

void pGETPID() /* ( -- u ) returns the MINFORTH PID , MM-110319 */
{  Push((Cell)getpid());
}

void pREADLINK() /* ( *path *buf bufsize -- -1|+n ) , MM-110319 */
{  Addr path, buf; Int size;
   Indepth(3); size = POP(), buf = POP(), path = TOS;
   TOS = readlink((char *)path,(char *)buf,size);
}

/* appdir() ( char_*buff -- strlen)             MM-190402  OSX  */
void pAPPDIR()
{
   /* printf("appdir:%s\n",appdir); fflush(stdout);  */
   Addr buff;
   Indepth(1); buff = TOS;
   strcpy((char *)buff,appdir);
   TOS = strlen(appdir);
}
...
void pUSLEEP() /* ( u -- ) read value n from address adr */
{  Indepth(1);
   usleep(TOS); DROP;
}

void pSYSCALL() /* ( x1..xn syscall-number n+1 -- y ) run Win32 API procedures */
{ Cell x1, x2, x3, x4, x5, n, scn;
   Indepth(2);
   n = Pop();
   scn = Pop();
   switch(n)
   {  case 1:  PUSH(syscall(scn));
               break;
      case 2:  Indepth(1);
               x1 = TOS;
               TOS = syscall(scn, x1);
      ...
      case 3:  Indepth(2);
               x2 = POP(); x1 = TOS;
               TOS = syscall(scn, x2, x1);
	       break;
      case 4:  Indepth(2);
               x3 = POP(); x2 = POP(); x1 = TOS;
               TOS = syscall(scn, x3, x2, x1);
	       break;
      case 5:  Indepth(2);
               x4 = POP(); x3 = POP(); x2 = POP(); x1 = TOS;
               TOS = syscall(scn, x4, x3, x2, x1);
      case 6:  Indepth(2);
               x5 = POP(); x4 = POP(); x3 = POP(); x2 = POP(); x1 = TOS;
               TOS = syscall(scn, x5, x4, x3, x3, x1);    
	       break;
      default: Throw(-21); }
}


void pDLOPEN() /* ( adr u flag -- hndl ior ) load dynamic library */
// {  Addr a; Int u, f;                                             MM-32/64
{  Addr a; Cell u, f;
   Indepth(3); f = POP(), u = 127 & TOS, a = SECOND;
   Inrange(a,u);
   memcpy(fname,PTR(a),u); fname[u]='\0';
// u = (Int) dlopen(fname,f);                                       MM-32/64
   u = (Cell) dlopen(fname,f);
   if (u == 0) TOS = -1l, SECOND = u;
   else        TOS = 0, SECOND = u;
}

void pDLCLOSE() /* ( hndl -- ior ) unload dynamic library */
{  Indepth(1);
   TOS = dlclose((void*)TOS);
}

void pDLSYM() /* ( adr u hndl -- a-addr ior ) return addr of symbol adr,u */
// {  Addr a; Int u, h;                                             MM-32/64
{  Addr a; Cell u, h;
   Indepth(3); h = POP(), u = 127 & TOS, a = SECOND;
   Inrange(a,u);
   memcpy(fname,PTR(a),u); fname[u]='\0';
//   a = (Int) dlsym((void*)h,fname);                               MM-32/64  
   a = (Cell) dlsym((void*)h,fname);
   if (a == 0) TOS = -1l, SECOND = a;
   else        TOS = 0, SECOND = a;
}

Cell (*pz)();
Cell (*pn)(Cell,...);
void pCALL() /* ( xn..x1 x-addr n -- y ) call library function x-addr */
{  Cell x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12;
   x1 = Pop(); /* printf("%ld",x1); fflush(stdout); */
   switch(x1)
   {  case 0:  Indepth(1);
               pz=(void*)POP();
      ...
      case 1:  Indepth(2);
               ...
               break;
      case 2:  Indepth(3);
               pn=(void*)POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2));
               break;
      case 3:  Indepth(4);
               pn=(void*)POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3));
               break;
      case 4:  Indepth(5);
               pn=(void*)POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4));
               break;
      case 5:  Indepth(6);
               pn=(void*)POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5));
               break;
      case 6:  Indepth(7);
               pn=(void*)POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6));
               break;
      case 7:  Indepth(8);
               pn=(void*)POP(), x7=POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6,x7));
               break;
      case 8:  Indepth(9);
               pn=(void*)POP(), x8=POP(), x7=POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6,x7,x8));
               break;
      case 9:  Indepth(10);
               pn=(void*)POP(), x9=POP(), x8=POP(), x7=POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6,x7,x8,x9));
               break;
      case 10:  Indepth(11);
               pn=(void*)POP(), x10=POP(), x9=POP(), x8=POP(), x7=POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10));
               break;
      case 11:  Indepth(12);
               pn=(void*)POP(), x11=POP(), x10=POP(), x9=POP(), x8=POP(), x7=POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11));
               break;
      case 12:  Indepth(13);
               pn=(void*)POP(), x12=POP(), x11=POP(), x10=POP(), x9=POP(), x8=POP(), x7=POP(), x6=POP(), x5=POP(), x4=POP(), x3=POP(), x2=POP(), x1=POP();
               PUSH(pn(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12));
               break;
      default: Throw(-21); }
}
...
/* MM-100809
   Signal handlers for GTK+. A signal handler is called with two arguments, the
   widget identifier of the widget that received the signal and the address
   of a data structure. With the object oriented gtk-api for MINFORTH the data
   structure is an instance of the class GSignalSlot. It holds the data that is
   required to connect a FORTH word to a GTK-Signal:

     +-------+
     |  hid  |  the signal handler identifier returned by g_signal_connect
     +-------+
     |   xt  |  xt = addr of the forth word to be called as signal handler
     +-------+
     |   0   |  makes the handlers VM return control to the Forth VM
     +-------+
     | data  |  the parameter, given to the signal connect command
     +-------+
     |  oid  |  identifier of the object, the signal handler is assigned to
     +-------+
*/

/* Default destroy signal handler for GtkObjects. MM-101221 */
void ppCB0 (Addr wid, Addr* oid)
{ 
  *oid=0;
}

void ppCB20 (Addr wid, Addr cbSlot)
{ /* printf("\n ppCB20: wid=%ld cbSlot=%ld ",wid,cbSlot); fflush(stdout); */
// *--stk=(AT(cbSlot+12));  /* push data */                       MM-32/64
   *--stk=(AT(cbSlot+(CellSize*3)));  /* push data */
// *--stk=(AT(cbSlot+16));  /* push oid  */                       MM-32/64
   *--stk=(AT(cbSlot+(CellSize*4)));  /* push oid  */
   RPUSH(IP);               /* save IP   */
// SETIP(cbSlot+4);         /* make IP point to second cell of cbSlot */
   SETIP(cbSlot+CellSize);                                    /*  MM-32/64  */
cb_loop:                    /* execute xt in a private VM  */
  W=AT(IP), IP_NEXT;
  if ( 0 == W )            /* leave VM when returning from xt */
     goto cb_end;
  ExecuteToken();
  goto cb_loop;
cb_end:
  SetIP(RPOP());           /* restore IP */
  /* printf("\n ppCB20: returning "); fflush(stdout); */
}

void ppCB30 (Addr wid, Cell x1, Addr cbSlot)
{ /* printf("\n ppCB31: wid=%ld x1=%ld cbSlot=%ld ",wid,x1,cbSlot); fflush(stdout); */
  *--stk=(x1);
  ppCB20(wid, cbSlot);
}

void ppCB40 (Addr wid, Cell x1, Cell x2, Addr cbSlot)
{ /* printf("\n ppCB31: wid=%ld x1=%ld x2=%ld cbSlot=%ld ",wid,x1,x2,cbSlot); fflush(stdout); */
  *--stk=(x1);
  *--stk=(x2);
  ppCB20(wid, cbSlot);
}

static unsigned long ppCB11 (Addr cbSlot)
{ /* printf("\n ppCB11: cbSlot=%ld ",cbSlot); fflush(stdout); */
  ppCB20(0, cbSlot);
  return labs(POP());
}


static unsigned long ppCB31 (Addr wid, Addr event, Addr cbSlot)
{ /* printf("\n ppCB31: wid=%ld event=%ld cbSlot=%ld ",wid,event,cbSlot); fflush(stdout); */
  *--stk=(event);
  ppCB20(wid, cbSlot);
  return labs(POP());
}
  
void pIO2CB () /* ( iomode -- C-addr_of_callback_handler|false ) */
{ Indepth(1);
  switch(POP())
  { case  0:  printf(" signal handler for io-mode 0 ") ;
//              PUSH((int)ppCB0);                                  MM-32/64
              PUSH((Cell)ppCB0);
              break;
    case 20:  /* printf(" signal handler for io-mode 20 ") */;
//              PUSH((int)ppCB20);
              PUSH((Cell)ppCB20);
              break;
    case 30:  /* printf(" signal handler for io-mode 30 ") */;
//              PUSH((int)ppCB30);
              PUSH((Cell)ppCB30);
              break;
    case 40:  /* printf(" signal handler for io-mode 40 ") */;
//              PUSH((int)ppCB40);
              PUSH((Cell)ppCB40);
              break;
    case 11:  /* printf(" signal handler for io-mode 11 ")*/;
//              PUSH((int)ppCB11);
              PUSH((Cell)ppCB11);
              break;
    case 31:  /* printf(" signal handler for io-mode 31 ")*/;
//              PUSH((int)ppCB31);
              PUSH((Cell)ppCB31);
              break;
    default:  PUSH(MFALSE); }
...
/* MM-32/64 */
void pCELL() /* ( -- cellsize ) returns the CellSize MM-170221 */
{  Push(CellSize);
}

/* MM-32/64 */
void pLONG_MIN() /* ( -- -n.max ) returns LONG_MIN     MM-170221 */
{  Push(LONG_MIN);
}
...
// if (tk == xtLIT) printf("%ld",AT(IP+4));                         MM-32/64
   if (tk == xtLIT) printf("%ld",AT(IP+CellSize));
...
Label2:
   ...
   case 'X': if (hexflag) { hexflag=0;	printf(" Decimal"); }
	           ...
	           goto Label1;
   case 'N': printf(" Nest");
	           if (trcrst > rst_max) trcrst--;
             break;
   case 'U': printf(" Unnest");
	           if (trcrst < rst_min) trcrst++;
             break;
   ...
   case 'B': printf(" Bye? Really? [Y] ");
             if (toupper(GetKey()) == 'Y') longjmp(fvm_buf,1);
             break;
...
{  Addr XT;
   if (W < PRIMNUMBER) {		/* inline primitives */
      (primfunc[W])(); return; }
// if ((W < 256)||(W >= totalsize))	/* unassigned */                 MM-32/64
   if ((W < (64*CellSize))||(W >= totalsize))	/* unassigned */
      Throw(-256);
   XT = AT(W);
   if (XT < EXECNUMBER) {		/* indirect primitives */
      (primfunc[XT])(); return; }
// if ((XT < 256)||(XT >= totalsize))	/* out of range */           MM-32/64
   if ((XT < (64*CellSize))||(XT >= totalsize))	/* out of range */
      Throw(-256);
